...
while true; do
  ...
  if [ "$(printf '%s' "$status" | jq 'length' 2>/dev/null || echo 1)" = "0" ]; then
    ...
    # NOT kill -TERM 1: on Fly, PID 1 is Fly's init, not our supervisor. Signal
    # process-compose directly; it traps SIGTERM, stops its children and exits,
    # so Fly's init sees the main command finish and stops the Machine.
    pkill -TERM process-compose \
      || pkill -TERM -f process-compose \
      || kill -TERM 1
    exit 0
  fi
done
