...
- VM size + swap live in `fly.toml` (`[[vm]]` and `swap_size_mb`) — opencode/Bun
  idles high (~800mb) and bursts, so we have to run above the free RAM tier
- Because the machine scales to zero most of the day, CPU+RAM are billed
  per-second only while awake, so even an oversized VM stays cheap (order of
  cents/month). Volume stays within the free 3GB.
